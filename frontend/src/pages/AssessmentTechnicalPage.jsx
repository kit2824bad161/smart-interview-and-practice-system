...
import {
  ...
  saveAssessmentTechnicalProgress,
  startAssessmentTechnical,
  submitAssessmentTechnical,
} from '../services/api';
...
export default function AssessmentTechnicalPage() {
  ...
  const {
    ...
    round: 'technical',
    onRestart: async () => {
      console.log('[FULLSCREEN] Restarting technical round on backend...');
      const res = await restartAssessmentRound('technical', { reason: 'FULLSCREEN_EXIT' });
  ...
  const handleFinalSubmit = useCallback(async () => {
    ...
    try {
      ...
      const result = await submitAssessmentTechnical({ responses });
      sessionStorage.setItem('smart_technical_result', JSON.stringify(result));

      // Intentionally exit fullscreen
      await exitFullscreenSafe();

      navigate('/assessment/technical/result');
    } catch (err) {
      ...
      if (err.response?.status === 409) {
        ...
        setError(err.response?.data?.message || 'Unable to submit your Technical test. Please try again.');
        setSubmitting(false);
  ...
  const loadAssessment = useCallback(async () => {
    try {
      ...
      console.log('[TECHNICAL] Restoring session...');

      const response = await getTechnicalSession();
      console.log('[TECHNICAL] API response:', response);
      const session = response?.session;

      if (!session) {
        console.log('[TECHNICAL] No active session found');
      ...
      if (remaining <= 0) {
        console.log('[TECHNICAL] Timer expired on load; auto-submitting once');
      ...
      console.error('[TECHNICAL] Restore failed:', err?.response?.status, err?.response?.data, err);
      if (err.response?.status === 403) {
        setIsLocked(true);
        setLockedReason(
          err.response?.data?.message ||
            'Minimum 15 out of 20 in Aptitude is required to qualify for the Technical Round.'
        );
        return;
      }
      if (err.response?.status === 409) {
      ...
      setError(err?.response?.data?.message || 'Unable to restore Technical MCQ test.');
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    console.log('[TECHNICAL] Component mounted');
    loadAssessment();
  }, [loadAssessment]);

  // Start new technical test with user-gesture fullscreen entry
  const handleEnterFullscreenAndStart = async () => {
    try {
      ...
      const data = await startAssessmentTechnical();
      ...
      setError(err.response?.data?.message || 'Failed to start Technical MCQ test.');
    } finally {
      setStarting(false);
    }
  };
  ...
  // Countdown timer
  useEffect(() => {
    if (loading || isLocked || secondsRemaining <= 0 || submitting || !questions.length) return;
  ...
  }, [isLocked, loading, questions.length, secondsRemaining, submitting]);

  // Save selected option and sync to server
  ...
  if (loading) {
    return (
      <div className="mx-auto flex min-h-[500px] max-w-4xl flex-col items-center justify-center">
        ...
        <p className="mt-4 font-semibold text-slate-600">Loading Technical Examination...</p>
        <p className="mt-1 text-xs text-slate-400">Verifying qualifications and preparing questions</p>
      </div>
    );
  }

  // Locked Banner if candidate bypassed or failed Round 1
  if (isLocked) {
    return (
      <div className="mx-auto max-w-xl py-16 text-center">
        ...
        <h2 className="mt-5 font-display text-2xl font-bold text-slate-900">
          Round 2 — Technical MCQ is Locked
        </h2>
        <p className="mt-3 text-sm leading-relaxed text-slate-600">
          {lockedReason || 'You must pass Round 1 (Aptitude) with at least 15 out of 20 marks to unlock this round.'}
        </p>
  ...
  if (error && !questions.length) {
    return (
      <div className="mx-auto max-w-xl py-16 text-center">
        ...
        <h2 className="mt-4 font-display text-2xl font-bold text-slate-800">Assessment Error</h2>
  ...
  if (noActiveSession && !questions.length) {
    return (
      <PreTestFullscreenNotice
        roundName="Round 2 – Technical MCQ Examination"
        roundNumber={2}
        passingScore={15}
        totalQuestions={20}
        durationMinutes={25}
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-4 md:py-6">
      ...
      <div className="flex flex-col justify-between gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-center">
        <div>
          ...
          <h1 className="mt-1 font-display text-2xl font-bold tracking-tight text-slate-900 md:text-3xl">
            Technical MCQ Examination
          </h1>
          <p className="text-xs text-slate-500">
            Answers and detailed technical explanations are revealed after test submission.
          </p>
        </div>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-4 text-xs font-bold text-slate-500">
        ...
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-slate-600">
            {currentQ?.topic || 'Software Engineering'}
          </span>
        ...
        <Card className="mt-6 p-6 md:p-9">
          ...
          <div className="mt-9 flex flex-wrap items-center justify-between gap-4 border-t border-slate-100 pt-6">
            <Button
              ...
              disabled={currentIndex === 0 || isRestarting}
            >
            ...
            <div className="flex items-center gap-3">
              {currentIndex < totalQuestions - 1 ? (
                ...
                <Button
                  ...
                  disabled={isRestarting}
                >
                  <Send size={16} /> Submit Technical Test
                </Button>
              )}
            </div>
          </div>
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <p className="text-xs font-bold uppercase tracking-wider text-slate-400">
            Technical Question Navigator (1 to 20)
          </p>
        ...
        <div className="mt-4 flex justify-end border-t border-slate-100 pt-3">
          <button
            ...
            onClick={() => setShowConfirmModal(true)}
            disabled={isRestarting}
            className="text-xs font-bold text-slate-500 hover:text-accent disabled:opacity-50"
          >
            Ready to finish? Click here to Submit Technical Test →
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <div className="flex items-center gap-3 text-amber-600">
              ...
              <h3 className="font-display text-lg font-bold text-slate-900">Submit Technical Examination</h3>
            </div>
            ...
            <p className="mt-3 text-sm leading-6 text-slate-600">
              {unansweredCount > 0 ? (
                ...
                'Are you sure you want to submit your Technical test? You cannot change your answers after submission.'
              )}
            </p>
      ...
      <FullscreenAssessmentGuard
        roundName="Technical MCQ Examination"
        roundNumber={2}
        passingScore={15}
        totalQuestions={20}
        durationMinutes={25}
