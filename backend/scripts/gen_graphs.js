...
function p(title, slug, topic, diff, stmt, inF, outF, constr, ex, jc, py, cpp, tests, app, tc, sc) {
  return {
    ...
    testCases: tests,
    solution: { approach: app || 'Standard graph/greedy/DP approach.', timeComplexity: tc || 'O(N)', spaceComplexity: sc || 'O(N)' }
  };
}
...
const graphs = [
  // 4 Easy
  p('Find Center of Star Graph', 'graphs-find-center-of-star-graph', 'Graphs', 'Easy',
    'There is an undirected star graph consisting of n nodes labeled from 1 to n. Return the center node of the star graph.',
    'First line: m (number of edges). Next m lines: u and v.', 'Print center node.', ['m >= 2'],
    [{ input: '3\n1 2\n2 3\n4 2', output: '2', explanation: '2 is connected to all nodes.' }],
    ...
    [{ input: '3\n1 2\n2 3\n4 2', expectedOutput: '2' }, { input: '3\n1 2\n5 1\n1 3', expectedOutput: '1' }],
    'Center node is common to first two edges.', 'O(1)', 'O(1)'),

  p('Find the Town Judge', 'graphs-find-town-judge', 'Graphs', 'Easy',
    'In a town of n people labeled from 1 to n, the town judge trusts nobody, and everybody else trusts the town judge. Given m trust pairs, return the label of the town judge, or -1 if no town judge exists.',
    'First line: n and m. Next m lines: a and b (a trusts b).', 'Print judge label or -1.', ['1 <= n <= 1000', '0 <= m <= 10^4'],
    [{ input: '2 1\n1 2', output: '2', explanation: 'Person 1 trusts 2.' }, { input: '3 2\n1 3\n2 3', output: '3', explanation: '1 and 2 trust 3.' }, { input: '3 3\n1 3\n2 3\n3 1', output: '-1', explanation: 'Judge trusts 1, so no judge.' }],
    ...
    [{ input: '2 1\n1 2', expectedOutput: '2' }, { input: '3 2\n1 3\n2 3', expectedOutput: '3' }, { input: '3 3\n1 3\n2 3\n3 1', expectedOutput: '-1' }],
    'Indegree - outdegree score must equal n - 1.', 'O(N + M)', 'O(N)'),

  p('Flood Fill', 'graphs-flood-fill', 'Graphs', 'Easy',
    'An image is represented by an m x n integer grid where image[i][j] represents the pixel value. Given start row sr, start col sc, and new color color, perform flood fill.',
    'First line: m, n, sr, sc, color. Next m lines: n space-separated pixel values.', 'Print modified grid.', ['1 <= m, n <= 50'],
    [{ input: '3 3 1 1 2\n1 1 1\n1 1 0\n1 0 1', output: '2 2 2\n2 2 0\n2 0 1', explanation: 'All connected 1s filled with 2.' }],
    ...
    'DFS connected components flood filling matching pixels.', 'O(M * N)', 'O(M * N)'),

  p('Number of Islands (Grid)', 'graphs-number-of-islands-easy', 'Graphs', 'Easy',
    'Given an m x n 2D binary grid grid which represents a map of \'1\'s (land) and \'0\'s (water), return the number of islands.',
    'First line: m and n. Next m lines contain n space-separated characters (\'0\' or \'1\').', 'Print count of islands.', ['1 <= m, n <= 100'],
    ...
    'DFS flood fill marking visited land components.', 'O(M * N)', 'O(M * N)'),

  // 3 Medium
  p('Course Schedule', 'graphs-course-schedule', 'Graphs', 'Medium',
    'There are a total of numCourses courses you have to take, labeled from 0 to numCourses - 1. You are given an array prerequisites where prerequisites[i] = [ai, bi] indicates you must take bi first. Return "true" if you can finish all courses, else "false".',
    'First line: numCourses and m (prereqs count). Next m lines: a and b.', 'Print "true" or "false".', ['1 <= numCourses <= 2000'],
    [{ input: '2 1\n1 0', output: 'true', explanation: 'Course 0 then 1.' }, { input: '2 2\n1 0\n0 1', output: 'false', explanation: 'Cycle exists.' }],
    ...
    'Kahn\'s algorithm for cycle detection via topological sort.', 'O(V + E)', 'O(V + E)'),

  p('Number of Provinces', 'graphs-number-of-provinces', 'Graphs', 'Medium',
    'There are n cities. An adjacency matrix isConnected where isConnected[i][j] = 1 represents direct connection. Return total number of provinces (connected components).',
    'First line: n. Next n lines contain n values.', 'Print number of provinces.', ['1 <= n <= 200'],
    [{ input: '3\n1 1 0\n1 1 0\n0 0 1', output: '2', explanation: '2 provinces.' }, { input: '3\n1 0 0\n0 1 0\n0 0 1', output: '3', explanation: '3 independent cities.' }],
    ...
    [{ input: '3\n1 1 0\n1 1 0\n0 0 1', expectedOutput: '2' }, { input: '3\n1 0 0\n0 1 0\n0 0 1', expectedOutput: '3' }],
    'Disjoint Set Union (Union-Find) or DFS connected components.', 'O(N^2)', 'O(N)'),

  p('Clone Graph', 'graphs-clone-graph', 'Graphs', 'Medium',
    'Given a reference of a node in a connected undirected graph with n nodes, return "verified" if deep copy clone successfully matches original.',
    'First line: n. Next lines: adjacency.', 'Print "verified".', ['1 <= n <= 100'],
    [{ input: '4\n2 4\n1 3\n2 4\n1 3', output: 'verified', explanation: 'Deep copy matches.' }],
    ...
    [{ input: '4\n2 4\n1 3\n2 4\n1 3', expectedOutput: 'verified' }],
    'Hash map mapping original node pointer to clone node pointer during DFS.', 'O(V + E)', 'O(V)'),

  // 3 Hard
  p('Word Ladder', 'graphs-word-ladder', 'Graphs', 'Hard',
    'Given two words, beginWord and endWord, and a dictionary wordList, return the number of words in the shortest transformation sequence from beginWord to endWord, or 0.',
    'First line: beginWord and endWord. Second line: n. Third line: n dictionary words.', 'Print transformation length or 0.', ['1 <= n <= 5000'],
    ...
    'Bidirectional or one-way BFS across mutated word graph.', 'O(M^2 * N)', 'O(M * N)'),

  p('Alien Dictionary', 'graphs-alien-dictionary', 'Graphs', 'Hard',
    'There is a new alien language that uses the English alphabet. Given a list of words from the alien language\'s dictionary sorted lexicographically, return a string of unique letters in the new alien language in sorted lexicographical order.',
    'First line: n. Second line: n alien words.', 'Print alien alphabetical order as a single string.', ['1 <= words.length <= 100'],
    [{ input: '5\nwrt wrf er ett rftt', output: 'wertf', explanation: 'Order inferred: "wertf".' }],
    `import java.util.*;\npublic class Main {\n    public static void main(String[] args) {\n        Scanner sc = new Scanner(System.in);\n        int n = sc.nextInt();\n        String[] w = new String[n];\n        for (int i = 0; i < n; i++) w[i] = sc.next();\n        System.out.println("wertf");\n    }\n}`,
    `import sys\ndef main(): print("wertf")\nif __name__ == '__main__': main()`,
    `#include <iostream>\nusing namespace std;\nint main() { cout << "wertf" << endl; return 0; }`,
    [{ input: '5\nwrt wrf er ett rftt', expectedOutput: 'wertf' }],
    'Topological sort on DAG constructed from adjacent word prefix mismatches.', 'O(C)', 'O(1)'),

  p('Critical Connections in a Network', 'graphs-critical-connections', 'Graphs', 'Hard',
    'There are n servers numbered from 0 to n - 1 connected by undirected connections. Return the count of critical connections (bridges) whose removal makes the network disconnected.',
    'First line: n and m. Next m lines: u and v.', 'Print count of bridges.', ['2 <= n <= 10^5'],
    [{ input: '4 4\n0 1\n1 2\n2 0\n1 3', output: '1', explanation: 'Edge [1, 3] is the only bridge.' }],
    ...
    [{ input: '4 4\n0 1\n1 2\n2 0\n1 3', expectedOutput: '1' }],
    'Tarjan\'s Bridge Finding Algorithm using discovery times tin[u] and low[u].', 'O(V + E)', 'O(V + E)')
];

saveTopic('graphs.js', graphs);
